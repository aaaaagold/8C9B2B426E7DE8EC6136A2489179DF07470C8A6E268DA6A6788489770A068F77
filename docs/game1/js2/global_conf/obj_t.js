...
$dddd$=$aaaa$.prototype._refresh=function f(){
	...
	if(c && $gameMap){
			if(c.parentId){
				let tevt=$gameMap._events[c.parentId];
				if(tevt){
					let sp=tevt._tmp._sprite;
					if(sp){
						sp._extendFrame(c._eventId,this._frame);
						this._parentNoted=true;
					}
				}
			}else if(c._sameStatEvts){
				let pw = ~~this.patternWidth();
				let ph = ~~this.patternHeight();
				let sx = ( ((this._tileId>>4)&8) + (this._tileId&7) ) * pw;
				let sy = ( (this._tileId>>3)&15 ) * ph;
				this._extendFrame('this',{x:sx,y:sy,width:pw,height:ph});
			}
	}
}; $dddd$.ori=$rrrr$;
$aaaa$.prototype._extendFrame=function(evtId,fr){
	let cfMap=this._cfMap,cf=this._cf;
	cfMap.set(evtId,fr);
	...
	let orifr=cfMap.get('this');
	let orix=(orifr.width>>1)+orifr.x;
	this.anchor.x=(orix-cf.xl)/(cf.width);
};
$rrrr$=$aaaa$.prototype.renderCanvas;
$dddd$=$aaaa$.prototype.renderCanvas=function f(renderer){
	if(this._parentNoted) return;
	return f.ori.call(this,renderer);
}; $dddd$.ori=$rrrr$;
