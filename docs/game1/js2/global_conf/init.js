...
0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
	...
	w.aes=((data,key,enc0dec1)=>{
		...
		function subWord(w,inv){
			let rtv=sbox(w,inv);
			for(let x=32;x-=8;) rtv|=sbox(w>>x,inv)<<x;
			return rtv;
		}
		function rotWord(w){ return w.rot_r(24); }
		function keyExp(arr_int){
			...
			for(let i=0,i_rcon=0;i!==R;++i){
				if(i<N) W.push(arr_int[i]);
					...
					let i_N=i%N;
					if(i_N==0) W.push( W[i-N]^subWord(rotWord(W[i-1]))^rcon(++i_rcon) );
					else if(i_N==4&&6<N) W.push( W[i-N]^subWord(W[i-1]) );
		...
		function addRoundKey(state,keys,round_n){
			//
			// state: arr_int
		...
		function mixCol(state){
			...
			for(let x=0,h,c,w;x!==4;++x){ // 4 cols
				...
				for(c=0;c!==4;++c){ // 4 bytes per int
					...
					b[c]=byte<<1;
					b[c]&=0xFF;
					//console.log(sh,w.toHexInt(1),byte,h,b[c].toString(2),(b[c]^(h&0x1B)).toString(2));
		...
		function aesblk(stat,keys){
				//console.log('e',stat.map(x=>x.toHexInt(1)));
			stat=addRoundKey(stat,keys,0);
				//console.log('e',stat.map(x=>x.toHexInt(1)));
			for(let r=1,rs=keys.length-2;r<=rs;++r){
				stat=stat.map(x=>subWord(x)); // SubBytes
					//console.log('e',stat.map(x=>x.toHexInt(1)));
				stat=shiftRow(stat);
					//console.log('e',stat.map(x=>x.toHexInt(1)));
				stat=mixCol(stat);
					//console.log('e',stat.map(x=>x.toHexInt(1)));
				stat=addRoundKey(stat,keys,r);
					//console.log('e',stat.map(x=>x.toHexInt(1)));
			}
				//console.log('e',stat.map(x=>x.toHexInt(1)));
			stat=stat.map(x=>subWord(x)); // SubBytes
			...
			stat=shiftRow(stat);
				//console.log('e',stat.map(x=>x.toHexInt(1)));
			stat=addRoundKey(stat,keys,keys.length-1);
				//console.log('e',stat.map(x=>x.toHexInt(1)));
			return stat;
			//return formatBlk(stat); // debug
		}
		
		function aesblk_dec(stat,keys){
			stat=addRoundKey(stat,keys,keys.length-1); //console.log(stat.map(x=>x.toHexInt(1)));
				//console.log('d',stat.map(x=>x.toHexInt(1)));
			stat=shiftRow_inv(stat);
				//console.log('d',stat.map(x=>x.toHexInt(1)));
			stat=stat.map(x=>subWord(x,1)); // SubBytes
				//console.log('d',stat.map(x=>x.toHexInt(1)));
			for(let r=keys.length-1;--r;){
				stat=addRoundKey(stat,keys,r);
					//console.log('d',stat.map(x=>x.toHexInt(1)));
				stat=mixCol_inv(stat);
					//console.log('d',stat.map(x=>x.toHexInt(1)));
				stat=shiftRow_inv(stat);
					//console.log('d',stat.map(x=>x.toHexInt(1)));
				stat=stat.map(x=>subWord(x,1)); // SubBytes
		...
		function aes(data,key){
			let keys=keyExp(parseKey(key)); //console.log('e','keys',keys.map(x=>x.map(x=>x.toHexInt(1))));
			...
			//console.log(blks);
			return blks.map(x=>aesblk(x,keys));
		}
		function aes_dec(data,key){
			let keys=keyExp(parseKey(key)); //console.log('e','keys',keys.map(x=>x.map(x=>x.toHexInt(1))));
			...
			//console.log(blks);
			return blks.map(x=>aesblk_dec(x,keys));
		}
		
		let z=Math.pow(2,35),r=/([0-9A-F][0-9A-F]?)/g;
		if(enc0dec1){
			let blks=aes_dec(data,key); // data is bytes
			let arr=[];
			for(let b=0;b!==blks;++b){
				for(let blk=blks[b],w=0;w!==blk.length;++w){
					for(let word=blk[w],x=32;x;){
						let last=arr.length;
						arr.push( (word>>(x-=8))&0xFF );
						if(arr[last]===0) return bytes2str(arr);
					}
				}
			}
